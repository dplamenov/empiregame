...
//database config
global $dbc;
$dbc = mysqli_connect('localhost', 'greatempire', 'greatempire', 'greatempire');
...
function deletearmy() {
    ...
    if ($get_finish_army >= 1) {
        while ($x = mysqli_fetch_assoc($get_finish_army)) {
            ...
            $bida = $x['building_id'];
            $userid = $x['user_id'];
            $select_army_sql = "SELECT * FROM building WHERE building_id ='" . $bid . "'";
            $select_army_q = mysqli_query($dbc, $select_build_sql);
            $select_army_a = mysqli_fetch_assoc($select_build_q);
